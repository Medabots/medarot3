...
patch_tilesets_ADDITIONAL := $(PATCH_TILESET_FILES) $(PATCH_TEXT_TILESET_FILES)\
 ...
 $(TILESET_OUT)/StartScreenMainLogo.$(COMPRESSED_TSET_TYPE)\
 $(TILESET_OUT)/StartScreenMainLogoRotation.$(COMPRESSED_TSET_TYPE)\
 $(TILESET_OUT)/StartScreenText.$(COMPRESSED_TSET_TYPE)\
 ...
 $(TILESET_OUT)/OpeningAnimationMedalLogo.$(COMPRESSED_TSET_TYPE)\
 $(TILESET_OUT)/OpeningAnimationCharacterNames.$(COMPRESSED_TSET_TYPE)\
 ...
 $(TILESET_OUT)/BorderTextMedarot3.$(COMPRESSED_TSET_TYPE)\
 $(TILESET_OUT)/RobattleTransition1.$(COMPRESSED_TSET_TYPE)\
 $(TILESET_OUT)/RobattleTransition2.$(COMPRESSED_TSET_TYPE)
...
$(BASE)/$(OUTPUT_PREFIX)%.$(ROM_TYPE): $(OBJECTS) $$(addprefix $(VERSION_OUT)/$$*., $$(addsuffix .$(INT_TYPE), $$(notdir $$(basename $$(wildcard $(SRC)/version/$$*/*.$(SOURCE_TYPE)))))) | $(BASE)/$(ORIGINAL_PREFIX)%.$(ROM_TYPE)
	...
	$(FIX) $(FIX_ARGS) -O -v -C -k 9C -l 0x33 -m 0x1B -p 0 -r 2 $@ -t "$(CURNAME)"
