...
for version in roms:
    with open(version[0], "rb") as rom, open(f"game/src/version/{version[1]}/tileset_table.asm", "w") as output:
        rom.seek(tiletable)
        ...
        #output.write('INCLUDE "game/src/version/tileset_table.asm"\n\n')
        ...
        while i < count:
            ...
            if namefile and realptr not in nametable:
                ...
                nametable[namesuffix] = f"Tileset{namesuffix:04X}"
                namefile.write(f"{namesuffix:04X}={nametable[realptr]}\n")
            # Skip empty entries
            if namesuffix in nametable:
                nametable[realptr] = nametable[namesuffix]
        ...
        for ptr in nametable:
            ...
            if realptr == terminator_key or realptr < 0x4000:
                continue
